...
const ContratoModal: React.FC<ContratoModalProps> = ({ plano, onClose }) => {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html>
        <head>
          <title>Contrato - Manutenção X</title>
          <style>
  ...
  const handleShare = async () => {
    const texto = `Contrato de Prestação de Serviços - Manutenção X\n\nPlano: R$${plano.preco}/mês (até ${plano.usuarios} usuários)\n\nPara mais detalhes, acesse: ${window.location.origin}`;

    if (navigator.share) {
      try {
        await navigator.share({ title: 'Contrato - Manutenção X', text: texto });
      } catch {
        /* user cancelled */
      }
    } else {
      ...
      <div className={styles.modal}>
        ...
        <div className={styles.modalBody} ref={contratoRef}>
          <div className={styles.contratoTexto}>
            ...
            <p className={styles.clausula}>
              A CONTRATADA disponibilizará à CONTRATANTE acesso completo ao sistema Manutenção X,
              incluindo todos os 22+ módulos disponíveis: Ordens de Serviço, Checklists de Manutenção, Quadro de Atividades,
            ...
            <p className={styles.clausula}>
              O software Manutenção X é de propriedade exclusiva da CONTRATADA.
