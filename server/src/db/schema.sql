-- ╔══════════════════════════════════════════════════════════════╗
-- ║  SCHEMA: Manutenção X — PostgreSQL                      ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- Registros de incidentes/atividades (usado por QR Code e Geolocalização).
-- NÃO confundir com o SLA de OS (sla_configuracoes + colunas sla_* em ordens_servico).
-- ════════════════════════════════════════════
...
-- ════════════════════════════════════════════
-- TABELA: respostas_qrcode
-- ════════════════════════════════════════════

CREATE TABLE respostas_qrcode (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- ── Calendário: anotações e legendas personalizadas ──
CREATE TABLE IF NOT EXISTS calendario_legendas (
  id SERIAL PRIMARY KEY,
  condominio_id UUID NOT NULL,
  cor VARCHAR(7) NOT NULL,
  rotulo VARCHAR(60) NOT NULL,
  criado_em TIMESTAMP DEFAULT NOW(),
  UNIQUE(condominio_id, cor)
);
...
CREATE INDEX IF NOT EXISTS idx_cal_anot_cond_data ON calendario_anotacoes(condominio_id, data);
CREATE INDEX IF NOT EXISTS idx_cal_leg_cond ON calendario_legendas(condominio_id);

-- ════════════════════════════════════════════
-- TABELA: checklists_locais (locais pré-cadastrados)
-- ════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS checklists_locais (
  id SERIAL PRIMARY KEY,
  condominio_id UUID REFERENCES condominios(id) ON DELETE CASCADE,
  nome VARCHAR(255) NOT NULL,
  itens_padrao TEXT[] DEFAULT '{}',
  criado_em TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_checklists_locais_cond ON checklists_locais(condominio_id);
...
-- ════════════════════════════════════════════
-- TABELA: metricas_uso (tracking de uso)
-- ════════════════════════════════════════════
