...
export const registerSchema = z.object({
  ...
  senha: z.string().min(6, 'Senha deve ter no mínimo 6 caracteres').max(128).regex(/^\d+$/, 'A senha deve conter apenas números'),
...
export const resetPasswordSchema = z.object({
  ...
  novaSenha: z.string().min(6, 'Senha deve ter no mínimo 6 caracteres').max(128).regex(/^\d+$/, 'A senha deve conter apenas números'),
});

export const changePasswordSchema = z.object({
  senhaAtual: z.string().min(1, 'Senha atual obrigatória').max(128),
  novaSenha: z.string().min(6, 'Nova senha deve ter no mínimo 6 caracteres').max(128).regex(/^\d+$/, 'A senha deve conter apenas números'),
});
...
export const portalChangePasswordSchema = z.object({
  ...
  nova_senha: z.string().min(6, 'Senha deve ter no mínimo 6 caracteres').max(128).regex(/^\d+$/, 'A senha deve conter apenas números'),
});
...
export const usuarioResetSenhaSchema = z.object({
  novaSenha: z.string().min(6).max(128).regex(/^\d+$/, 'A senha deve conter apenas números'),
});
